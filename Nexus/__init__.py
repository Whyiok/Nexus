...
def accept(contentType, id):
    if contentType == 'post':
        ...
        new_notice = Notification(user_id=post.id_author, from_user_id=current_user.id,
            post_id=post.id, type="ваш пост был принят!", message="")
        ...
        new_notice = Notification(user_id=discuss.id_author, from_user_id=current_user.id,
            discuss_id=discuss.id, type="ваша дискуссия была принята!", message="")
        db.session.add(new_notice)
        db.session.commit()
    else:
...
def deny(contentType, id):
    if contentType == 'post':
        ...
        db.session.commit()
        new_notice = Notification(user_id=post.id_author, from_user_id=current_user.id,
            post_id=post.id, type="ваш пост был отклонен.", message="")
        db.session.add(new_notice)
        db.session.commit()
    elif contentType == 'discuss':
        discuss = Discuss.query.filter_by(id=id).first()
        ...
        db.session.commit()
        new_notice = Notification(user_id=discuss.id_author, from_user_id=current_user.id,
            discuss_id=discuss.id, type="ваша дискуссия была отклонена.", message="")
        db.session.add(new_notice)
        db.session.commit()
    else:
        flash('Неизвестный тип контента.', 'danger')
