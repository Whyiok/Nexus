...
def send_email(verifyType):
    if request.method == 'POST':
        ...
        # Для логина проверяем существование пользователя и пароль
        if verifyType == 'login':
            ...
            # Проверка пароля ДО отправки email
            if not check_password_hash(user.hash_password, password):
                flash('Неверный пароль!', 'danger')
                return redirect(url_for('login'))
            
            username = user.username  # Берем имя из БД
        ...
        # Удаляем старые неактивированные коды для этого email
        Email_cods.query.filter_by(email=email, is_activated=False).delete()
        db.session.commit()

        # Проверяем, есть ли уже активированный код
        existing_activated_code = Email_cods.query.filter_by(
            email=email, is_activated=True).first()
        
        if existing_activated_code:
            # Если есть активированный код, используем его
            code = existing_activated_code.code
        else:
            # Генерируем новый код
            code = generate_code()
            new_code = Email_cods(email=email, code=code, is_activated=False)
...
Подтвердите свою электронную почту, введя код ниже:
{code}
...
Если вы не запрашивали вход, проигнорируйте это сообщение.'''
                ...
                msg = Message(
                    subject="Nexus MC: Подтверждение регистрации",
                    recipients=[email],
                    body=f'''Привет, {username}!
...
Добро пожаловать в наше сообщество!'''
            ...
            mail.send(msg)
            return '', 204  # Успешная отправка
            
        except Exception as e:
            print(f"Ошибка отправки email: {e}")
            flash('Не удалось отправить код подтверждения. Попробуйте позже.', 'danger')
            return redirect(url_for('register' if verifyType == 'register' else 'login'))

    return '', 400  # Bad request if not POST
    ...
    if posts:
        for post in posts:
            ...
            if current_user.is_authenticated:
                ...
                post.liked = False
    return render_template("servers.html", posts=posts)
